...
export function createBoothTranslationItems(booths) {
  const items = booths.map((booth, idx) => ({
    entity_type: "booth",
    entity_id: (
      (booth && (booth.booth_id || booth.id))
        ? (booth.booth_id || booth.id).toString()
        : `booth-temp-${idx}`
    ),
    fields: [
      {
      ...
      ...(booth.location?.name
        ...
        : []),
      // corners 배열이 있을 경우 CornerName_<i> 필드로 추가
      ...(Array.isArray(booth.corners) && booth.corners.length > 0
  ...
  // 디버그: 개발 환경에서만 생성된 항목을 로그로 남깁니다.
  if (import.meta.env.MODE !== "production") {
    console.debug("createBoothTranslationItems -> items:", items);
  }
  return items;
}
...
export function createBoothMenuTranslationItems(menus, boothId) {
  ...
  const item = {
    entity_type: "booth",
    entity_id: boothId ? boothId.toString() : "booth-temp-menu",
    fields,
  };

  if (import.meta.env.MODE !== "production") {
    console.debug("createBoothMenuTranslationItems -> item:", item);
  }
